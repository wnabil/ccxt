...
class pionex(Exchange, ImplicitAPI):
    ...
    async def fetch_trades(self, symbol: str, since: Int = None, limit: Int = None, params={}) -> List[Trade]:
        ...
        request: dict = {
            'symbol': self.market_api_symbol(market['id']),
        }
    ...
    async def fetch_ohlcv(self, symbol: str, timeframe='1m', since: Int = None, limit: Int = None, params={}) -> List[list]:
        ...
        if since is not None:
            ...
            request['endTime'] = min(self.sum(since, duration * limit * 1000), self.milliseconds())
        response = await self.publicGetMarketKlines(self.extend(request, params))
        data = self.safe_dict(response, 'data', {})
        klines = self.safe_list(data, 'klines', [])
        # {
        # "result": True,
        # "data": {
        #     "klines": [
    ...
    async def fetch_tickers(self, symbols: Strings = None, params={ }) -> Tickers:
        ...
        if params['type'] is None:
            params['type'] = 'SPOT'
        request = params
        response = await self.publicGetMarketTickers(request)
        # {
        # "data": {
        #     "tickers": [
        #     {
        ...
        # "timestamp": 1566691672311
        # }
        response2 = await self.publicGetMarketBookTickers(request)
        # {
        # "data": {
        #     "tickers": [
        #     ]
        ...
        if symbols is None:
            ...
            for i in range(0, len(self.symbols)):
                ...
                symbolsChanged.append(self.market_api_symbol(symbols[i]))
        tickers = self.filter_by_array(tickers, 'symbol', symbolsChanged)
        tickers2 = self.filter_by_array(tickers2, 'symbol', symbolsChanged)
        tickersFinal = []
        unmatchedTickers = []
        for i in range(0, len(tickers)):
            ...
            for j in range(0, len(tickers2)):
                ...
                if ticker2['symbol'] == ticker['symbol']:
                    ...
                    tickersFinal.append(ticker2)
        for i in range(0, len(unmatchedTickers)):
            ticker = unmatchedTickers[i]
            tickersFinal.append(ticker)
        tickersFinal = self.filter_by_array(tickersFinal, 'symbol', self.symbols)
        return self.parse_tickers(tickersFinal, self.symbols)
    ...
    async def fetch_ticker(self, symbol: str, params={}) -> Ticker:
        ...
        request: dict = {
            'symbol': self.market_api_symbol(market['id']),
            'type': self.safe_string_upper(market, 'type'),
        }
        ...
        tickers = self.safe_list(data, 'tickers', [])
        tickers2 = self.safe_list(data2, 'tickers', [])
        return self.parse_ticker(self.extend(tickers[0], tickers2[0]), market)
    ...
    def handle_errors(self, code: int, reason: str, url: str, method: str, headers: dict, body: str, response, requestHeaders, requestBody):
        ...
        if not self.safe_bool(response, 'result', False):
            feedback = self.id + ' ' + self.json(response)
            self.throw_exactly_matched_exception(self.exceptions, self.safe_string(response, 'code'), feedback)
    ...
    def market_api_symbol(self, symbol: Str):
